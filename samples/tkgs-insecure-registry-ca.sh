...
Mandatory arguments 
        ...
        --vc_ip
        --sv_ip
	--ca_file  (optional, filename only,  copy to /etc/ssl/certs/)
        -h, --help                        show help.
        
        Example:
        ./tkg-insecure-registry.sh -c \${cluster_name} -n \${namespace} -r \${url_registry} --vc_admin_passowrd \${admin_pass} --vc_admin_user \${admin_user} --vc_ip \${vc_ip} --sv_ip \${supervisor_cliuster_ip} --vc_root_password \${root_pass} --ca_file harbor-ca.crt"
}
...
CA_FILE="" # optional. harbor-ca.crt. put the file in current directory.
...
# MODIFIEE-BEGIN
if [[  -z "$CA_FILE" ]]; then 
  loginfo "No $CA_FILE provided, skipping";
else

## copy harbor-root CA to Supervisor Master VM.
sshpass -p "${SV_MASTER_PASSWORD}" scp -q -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null ./$CA_FILE root@"${SV_MASTER_IP}":/tmp/$CA_FILE


while read -r IPS_NODES_READ;
do

loginfo "Copying $CA_FILE  to '"${IPS_NODES_READ}"'..."
sshpass -e ssh  -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null  -T -t root@"${SV_MASTER_IP}" /bin/bash << EOF
scp  -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -i ./tkc-ssh-privatekey /tmp/"$CA_FILE"  vmware-system-user@"${IPS_NODES_READ}":/tmp 
EOF

loginfo "  Adding CA to system path  the node '"${IPS_NODES_READ}"'..."
...
sudo -i
rm -rf /etc/ssl/certs/"$CA_FILE" && cp /tmp/"$CA_FILE" /etc/ssl/certs/
#cat /tmp/harbor-root.crt >> /etc/pki/tls/certs/ca-bundle.crt
ls -al /etc/ssl/certs/"$CA_FILE"

#Verify that the change was added successfully. If it was, replace daemon.json. If not, exit without copying.
#if [[ -f "/etc/ssl/certs/$CA_FILE" ]]; exit 0; else exit 2; fi
...
then
  ...
  logerr "  There was an error adding file $CA_FILE.  Exiting..."
  exit 2
fi
done < "./ip-nodes-tkg"
